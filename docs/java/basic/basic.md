基础
使用常见类型的数据并编写基本语法。
Swift 是一种用于 iOS、macOS、watchOS 和 tvOS 应用程序开发的编程语言。如果您有使用 C 或 Objective-C 进行开发的经验，那么您将会熟悉 Swift 的许多部分。

Swift 提供了所有基本 C 和 Objective-C 类型的自己的版本，包括Int整数、Double浮点Float值、Bool布尔值和String文本数据。Swift 还提供了三种主要集合类型 、Array、Set和的强大版本Dictionary，如集合类型中所述。

与 C 一样，Swift 使用变量通过识别名称来存储和引用值。Swift 还大量使用其值无法更改的变量。这些被称为常量，比 C 中的常量强大得多。在整个 Swift 中都使用常量，以便在您使用不需要更改的值时使代码更安全、意图更清晰。

除了熟悉的类型之外，Swift 还引入了 Objective-C 中没有的高级类型，例如元组。元组使您能够创建和传递值分组。您可以使用元组将函数中的多个值作为单个复合值返回。

Swift 还引入了可选类型，用于处理值的缺失。可选表示“有一个值，并且它等于x ”或“根本没有值”。使用可选值与 Objective-C 中的指针类似nil，但它们适用于任何类型，而不仅仅是类。可选值不仅比nilObjective-C 中的指针更安全、更具表现力，而且是 Swift 许多最强大功能的核心。

Swift 是一种类型安全的语言，这意味着该语言可以帮助您清楚代码可以使用的值的类型。如果您的部分代码需要 a String，类型安全可以防止您错误地传递它Int。同样，类型安全可以防止您意外地将可选值传递String给需要非可选值的代码String。类型安全可帮助您在开发过程中尽早发现并修复错误。

常量和变量
常量和变量将名称（例如或）与特定类型的值（例如数字或字符串）相关联。常量的值一旦设置就无法更改，而变量可以在将来设置为不同的值。maximumNumberOfLoginAttemptswelcomeMessage10"Hello"

声明常量和变量
常量和变量必须在使用前声明。您可以使用关键字声明常量let，并使用关键字声明变量var。以下是如何使用常量和变量来跟踪用户尝试登录的次数的示例：

let maximumNumberOfLoginAttempts = 10
var currentLoginAttempt = 0
这段代码可以读作：

“声明一个名为 的新常量，并为其赋予值。然后，声明一个名为 的新变量，并为其指定初始值。”maximumNumberOfLoginAttempts10currentLoginAttempt0
...
如果代码中存储的值不会更改，请始终使用关键字将其声明为常量let。仅使用变量来存储需要能够更改的值。

类型注释
您可以在声明常量或变量时提供类型注释，以明确常量或变量可以存储的值的类型。通过在常量或变量名称后面放置一个冒号，后跟一个空格，然后是要使用的类型的名称来编写类型注释。

此示例为名为 的变量提供类型注释，以指示该变量可以存储值：welcomeMessageString

var welcomeMessage: String
声明中的冒号表示“...类型...”，因此上面的代码可以读作：

“声明一个名为that 的类型的变量。”welcomeMessageString

短语“of type String”的意思是“可以存储任何String值”。将其视为可以存储的“事物类型”（或“事物种类”）。

现在可以将该变量设置为任何字符串值而不会出现错误：welcomeMessage
...
在此示例中，如果没有干净的菜肴或缺少任何成分，该函数将抛出错误。因为可能会引发错误，所以函数调用被包装在表达式中。通过将函数调用包装在语句中，引发的任何错误都将传播到提供的子句。makeASandwich()makeASandwich()trydocatch
...
断言和前提条件
断言和前提条件是在运行时发生的检查。您可以使用它们来确保在执行任何进一步的代码之前满足基本条件。如果断言或前置条件中的布尔条件的计算结果为true，则代码执行将照常继续。如果条件评估为false，则程序的当前状态无效；代码执行结束，您的应用程序被终止。
...
assert(age >= 0)
如果代码已经检查了条件，则可以使用该函数来指示断言失败。例如：assertionFailure(_:file:line:)
...
您可以通过调用该函数来编写前提条件。您可以向该函数传递一个计算结果为or 的表达式，以及一条在条件结果为 时显示的消息。例如：precondition(_:_:file:line:)truefalsefalse

// In the implementation of a subscript...
precondition(index > 0, "Index must be greater than zero.")
您还可以调用该函数来指示发生了故障 - 例如，如果采用了开关的默认情况，但所有有效输入数据都应由开关的其他情况之一处理。preconditionFailure(_:file:line:)
